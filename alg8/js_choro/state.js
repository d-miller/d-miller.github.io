...
var quantize = d3.scale.threshold()
    .domain([.1001, .2001, .3001, .4001, .5001])
    .range(d3.range(6).map(function(i) { return "hasColor stateMap q" + i + "-9"; }));
/*colors = ["#c6dbef", "#9ecae1", "#6baed6", "#3182bd", "#08519c"];
var colorScale = d3.scale.threshold()
    .domain([.1001, .2001, .4001, .7001])
    .range(colors);*/
...
var legend = svg.append("g")
                .classed("legend", true)
                .attr("transform", "translate(" + (width-legW-30) + "," + (height-legH-15) + ")");
...
legend.selectAll("circle.gradient")
      ...
      .attr("class", function(i) {
        return "hasColor stateMap q" + i + "-9"; 
      })
...
var agg = "state";
queue().defer(d3.json, "shape/county.json")
       .defer(d3.csv, "data/map_" + agg + ".csv")
       .await(ready);
...
function ready(error, shapes, data) {
  ...
  svg.append("g")
      ...
      .attr("id", "stateGrp")
    .selectAll("path")
      .data(topojson.feature(shapes, shapes.objects.states).features)
    .enter().append("path")
 ...
 svg.insert("g", "#stateGrp")
  ...
  //if districts...
  if (agg == "district") {
    d3.json("district.json", function(error, districts) {
      svg.insert("g", "#stateGrp")
        .attr("class", "counties")
      .selectAll("path")
